			...
			if ( v.subvideos ) {
				...
				sub = (
					<div className="sub">
						...
						<div dangerouslySetInnerHTML={ { __html: v.excerpt } } />
					</div>
				);
			}
			...
			return (
				<section key={ v.id }>
					<div className="full">
						...
						<h1 dangerouslySetInnerHTML={ { __html: v.title + '<br>[Full Video]' } } />
...
registerBlockType( 'morphatic/if-incourage-topic-selector', {
	...
	save: props => {
		...
		if ( Object.keys( videos ).length > 0 ) {
			...
			const tabs = Object.keys( videos ).map( id => {
			...
			const content = Object.keys( videos ).map( tab => {
				...
				return (
					<div className="videos" key={ tab.id }>{ vids }</div>
				);
			} );
